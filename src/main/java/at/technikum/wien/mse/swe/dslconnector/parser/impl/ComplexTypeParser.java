    ...
    public <T> T parseValue(final String source) throws FieldParserException {
        ...
        if (!allPresent) {
            final String err = "at least one of the required fields in the list of ComplexTYpe names is not available in the class \n" +
                    ...
                    "available field in class " + t.getClass().getSimpleName() + ": \n" +
        ...
        final Map<Field, FieldParser> fieldParsers = complexTypeDtos.entrySet().stream()
                .map(entry -> {
                    ...
                    final SimpleTypeParser simpleTypeParser = new SimpleTypeParser(f.getType(), simpleTypeDto);
                    return new AbstractMap.SimpleEntry<>(f, simpleTypeParser);
                })
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

        // final T obj  = (T)parser.parseComplexTypeObject(source, (Class) t, fields, fieldParsers);
